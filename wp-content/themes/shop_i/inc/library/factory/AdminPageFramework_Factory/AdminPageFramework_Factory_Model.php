 ...
 */
abstract class bellaAdminPageFramework_Factory_Model extends bellaAdminPageFramework_Factory_Router {
    ...
    protected function _loadFieldTypeDefinitions() {
        if (empty(self::$_aFieldTypeDefinitions)) {
            self::$_aFieldTypeDefinitions = bellaAdminPageFramework_FieldTypeRegistration::register(array(), $this->oProp->sClassName, $this->oMsg);
        }
    ...
    protected function _registerField(array $aField) {
        bellaAdminPageFramework_FieldTypeRegistration::_setFieldResources($aField, $this->oProp, $this->oResource);
        if ($aField['help']) {
    ...
    protected function _isValidationErrors() {
        if (isset($GLOBALS['abellaAdminPageFramework']['aFieldErrors']) && $GLOBALS['abellaAdminPageFramework']['aFieldErrors']) {
            return true;
        }
    ...
    public function _replyToSaveFieldErrors() {
        if (!isset($GLOBALS['abellaAdminPageFramework']['aFieldErrors'])) {
            return;
        }
        $this->oUtil->setTransient("apf_field_erros_" . get_current_user_id(), $GLOBALS['abellaAdminPageFramework']['aFieldErrors'], 300);
    }
    public function _replyToSaveNotices() {
        if (!isset($GLOBALS['abellaAdminPageFramework']['aNotices'])) {
            return;
        }
        if (empty($GLOBALS['abellaAdminPageFramework']['aNotices'])) {
            return;
        }
        $this->oUtil->setTransient('apf_notices_' . get_current_user_id(), $GLOBALS['abellaAdminPageFramework']['aNotices']);
    }
