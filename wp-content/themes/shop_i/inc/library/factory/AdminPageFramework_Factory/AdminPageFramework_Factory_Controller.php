 ...
 */
abstract class bellaAdminPageFramework_Factory_Controller extends bellaAdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['abellaAdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('abellaAdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['abellaAdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['abellaAdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['abellaAdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['abellaAdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['abellaAdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['abellaAdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('abellaAdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['abellaAdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['abellaAdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['abellaAdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('abellaAdminPageFramework', 'aNotices'), array());
        if (!$sType) {
